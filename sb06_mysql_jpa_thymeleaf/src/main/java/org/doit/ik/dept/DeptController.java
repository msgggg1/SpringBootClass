...
public class DeptController {
	...
	*/
	...
	public ModelAndView deptList() {
		...
		Page<Dept> deptPage = deptService.getDeptsByPage(0, 3);
		
		List<Dept> depts = deptPage.getContent();
		mv.addObject("depts", depts);
		
		mv.addObject("totalPages", deptPage.getTotalPages());
		mv.addObject("totalElements", deptPage.getTotalElements());
		mv.addObject("hasNext", deptPage.hasNext());
